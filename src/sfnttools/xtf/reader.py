...
class XtfReader(SfntReader):
    @staticmethod
    def create(stream: Stream, verify_checksum: bool) -> 'XtfReader':
        stream.seek(0)
        table_directory = TableDirectory.parse(stream)
        return XtfReader(stream, table_directory, 0, None, False, verify_checksum)

    @staticmethod
    def create_by_ttc(stream: Stream, font_index: int, verify_checksum: bool) -> 'XtfReader':
        stream.seek(0)
        header = TtcHeader.parse(stream)
        table_directory, table_directory_offset = header.read_table_directory(stream, font_index)
        return XtfReader(stream, table_directory, table_directory_offset, None, False, verify_checksum)
...
class XtfCollectionReader(SfntCollectionReader):
    @staticmethod
    def create(stream: Stream, share_tables: bool, verify_checksum: bool) -> 'XtfCollectionReader':
        stream.seek(0)
        header = TtcHeader.parse(stream)
        return XtfCollectionReader(stream, header, share_tables, verify_checksum)
